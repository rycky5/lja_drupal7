...
class LeitorXML {
  protected $title;
	...
	function __construct($file){
                ...
                $strArquivo = trim(file_get_contents($file));
			
                $arqTxt = fopen("rss.xml", "w");

                fwrite($arqTxt, $strArquivo);

                fclose ($arqTxt);
          
		$this->info = simplexml_load_file("rss.xml");
	...
	function reader(){
		...
		for ($i = 0; $i < 3; $i++){
                    ...
                    echo '<h2>' . date('d/m/Y') . '</h2>';
                     echo "<p>" .str_replace("[@#galeria#@]", "", str_replace("[@#podcast#@]", "", str_replace("[@#video#@]", "",str_replace('##RECOMENDA##','',$this->textoNews))))."</p><br>";                    echo "</li>";
		}
                echo"</ul>";
	}
}
