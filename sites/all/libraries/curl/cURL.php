...
/**
 * Class cURL
 * Usage for manipulate browser using libcurl
 * + Fixed bug in get method.
 *
 * Workaround: Contains fix of Content-Type in POST with file upload.
 *
 * @author Marcelo Bur�gio
 * @subpackage util
 * @version 2.3
 * @since 14/12/2009 09:51:00
 */
class cURL {
	/**
	 * Resource cURL
	 ...
	 * @return string
	 */
	public function get($strUrl, $arrData=array(), $intRetries=1, $bolIsAspX=false, $enconding = false) {
		if (!empty($arrData) && is_array($arrData)) {
			$strUrl .= (strpos($strUrl, "?") !== false) ? "?" : "&";
			$strData = http_build_query($arrData);
			// Replace array data to single line (only ASP.net)
			if ($bolIsAspX) $strData = preg_replace("/%5B([0-9]+)%5D=/i", "=", $strData);
			$strUrl .= $strData;
		}
		...
		$this->setup($strUrl);
                ...
                if($enconding)
                  curl_setopt($this->ch,CURLOPT_ENCODING , $enconding);
                
		// Reseting POST configuration
		...
		for ($intCount = 1; $intCount <= $intRetries; $intCount++) {
			...
			if ($strResponse == false) $this->strErrorMessage = curl_error($this->ch) ." (". curl_errno($this->ch) .")";
			else {
				$this->strErrorMessage = "";
				break;
			}
	...
	public function post($strUrl, $arrData=array(), $intRetries=1, $bolIsAspX=false) {
		...
		if (!empty($arrData) && is_array($arrData)) {
			...
			if ($bolFileExists) {
				...
				if (!$this->postWithFiles($arrData)) return false;
			}
			else {
				$strData = http_build_query($arrData);
				// Replace array data to single line (only ASP.net)
		...
		foreach ($arrData as $strName => $strData) {
			...
			if (preg_match("/@([^;]*)(;type=(.*))?/", $strData, $arrFile)) {
				...
				if (!empty($strFileName)) {
					if (file_exists($strFileName)) $bolFileContents = true;
					else {
	...
	public function getCookieFile() {
		if (empty($this->strCookieFile)) $this->setCookieFile();
	...
	public function getUserAgent() {
		if (empty($this->strUserAgent)) $this->setUserAgent();
	...
	public function processarInputs($strConteudo) {
		...
		foreach ($arrConteudo[1] as $strCampo) {
			...
			$strValorCampo = "";
			$strTipoCampo  = "";
			if (preg_match('/name="([^"]*)"/i',  $strCampo, $arrCampo)) $strNomeCampo  = $arrCampo[1];
			if (preg_match('/value="([^"]*)"/i', $strCampo, $arrCampo)) $strValorCampo = $arrCampo[1];
