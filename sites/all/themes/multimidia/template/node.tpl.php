...
<!--
Aki está o dump
<? //var_dump($node->field_pagina["und"][0]['value']);?>
-->
...
<link rel="stylesheet" href="<?= base_path(true) ?>/sites/all/themes/carnaval/css/jquery-ui-1.8.17.custom.css" type="text/css" media="all" />

<h5 class="autorNoticia" style="border: 0px;"><?=@$node->field_chapeu["und"][0]['value']?></h5>
<h2 class="tituloNoticia"><span itemprop="name"><?= $title ?></span></h2>
<h3 class="descricaoNoticia"><span itemprop="description"><?= $node->body[$node->language][0]['summary']; ?></span></h3>
<h5 class="autorNoticia">
 <?
  //tava chamando a função semAcentos()  
  $file = drupal_get_path('theme', 'leiaja').'/images/'.$node->field_fonte[$node->language][0]['value'].'.jpg';
...
<div id="divConteudoExibir">
    ...
    <div style="float:left;">
      <link type="text/css" rel="stylesheet" media="all" href="<?= base_path().drupal_get_path('theme', 'espparques'); ?>/css/spacegallery.css" />
    <?php
      if(!empty($node->field_videost["und"][0]["value"])){
    ...
    <?php if(!empty($node->field_videost["und"][0]["value"])): ?>
      <div class="detalhesVideo">
    	<span>Embed</span>
        <div class="bgInputEmbed">
            <input name="embed" type="text" value='<?= $node->field_videost["und"][0]["value"] ?>' onclick="this.focus();this.select();" />
        </div>
        <span class="views">Views:<strong><?= api_view_videos($node->field_videost["und"][0]["value"]) ?></strong></span>
      </div>
    <?php endif; ?>

    <?php if(!empty($node->field_image[$node->language])): ?>
      <div class="detalhesVideo">
    	<span>Embed</span>
        <div class="bgInputEmbed">
...
//  })
  ...
  function esconder(){
    $("#divConteudoExibir,.tagsExibir,#divCorrigir,#divRecomendar,#divFaleconosco").hide();
  }
