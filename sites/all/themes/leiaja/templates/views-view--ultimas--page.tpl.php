...
?>
  ...
  <h4><a href="<?= $base_url.url(drupal_lookup_path('alias',"node/".$value->nid)); ?>"><?=$value->node_title;?></a></h4>
  <p>
    <a href="<?= $base_url.url(drupal_lookup_path('alias',"node/".$value->nid)); ?>">
  ...
  <div class="tagsExibir">
    ...
    <ul class="tags">
      <?foreach($value->field_field_tags AS $tKey => $tValue){
        ?>
        <li><a href="<?=$base_url.url(drupal_lookup_path('alias',"taxonomy/term/".$tValue["rendered"]["#options"]["entity"]->tid));?>" title="<?=$tValue["rendered"]["#title"]?>"><?=$tValue["rendered"]["#title"]?></a></li>
      <?}?>
    </ul>
  </div>
</li>
