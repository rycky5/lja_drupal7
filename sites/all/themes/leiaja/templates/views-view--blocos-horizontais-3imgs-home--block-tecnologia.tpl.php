...
?>
<div class="colunas3">
  <h2 class="tituloH2"><a href="/tecnologia" class="azulClaro" title="Tecnologia">Tecnologia</a></h2>

<?php
$resultViews = views_get_view_result('blocos_horizontais_3imgs_home', 'block_tecnologia',api_getNidIgnore());
...
foreach ($resultViews as $key => $value){
    ...
    $tid = $value->_field_data['nid']['entity']->field_cattecnologia['pt-br'][0]['tid'];
    ...
    if(!empty($ImageCapa)){
        $uriImage = $ImageCapa[0]['rendered']['#item']['uri'];
    }else{
...
<div class="contentCol <?php print $classeMargem;?>">
    ...
    <strong><a href="<?= url(drupal_lookup_path('alias',"taxonomy/term/".$tid)); ?>" class="linksStrong azulClaro"><?=$subcategoria ?></a></strong>
