...
<h4><a href="<?= url('feed2/rss/', array('absolute' => TRUE)) ?>" title="Leia J&aacute;"><img src="<?= LEIAJAURL ?>/sites/all/themes/leiaja2/images/rss.png" width="32" /><span>Leia J&aacute;</span></a></h4>
...
foreach ($array_obj_vocabulary as $vocabulary) {
    ...
    if ($cont % 3 == 0) {
        ...
        <div class="listaRss">
            <h5>
                <a href="<?= base_path() . 'feed2/rss/' . $vocabulary->machine_name; ?>" title="<?= $vocabulary->name; ?>"><img src="<?= LEIAJAURL ?>/sites/all/themes/leiaja2/images/rss.png" width="24" /><span><?= $vocabulary->name; ?></span></a>
            </h5>
            <ul>
                <?php
                ...
                foreach ($terms as $term) {
                    ...
                    //var_dump($alias);
                    ?>
                    <li><a href="<?= base_path() . 'feed2/rss/' . $alias ?>" title="<?= $term->name ?>"><?= $term->name ?></a></li>
                            <?php
                        }
                        ?>
            </ul>
